<aside class="fixed inset-y-0 left-0 z-30 hidden w-64 bg-surface border-r border-border lg:block">
    ...
    <div class="px-6 pb-4">
        <div class="flex items-center gap-2">
            ...
            <span class="text-xs px-2 py-1 rounded-full bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200">{{ auth()->user()->role }}</span>
        </div>
    </div>
    ...
    <nav class="px-4 py-4 space-y-1">
        ...
        <a href="{{ route('calendar.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-md text-foreground/90 hover:bg-muted/50 hover:text-foreground transition-colors">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
